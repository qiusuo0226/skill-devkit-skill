...
> - 首提交
> - 建 `governance/`（版本控制、基线、升级记录、打包、治理提示词，不进分发包）
> - 打 `0.1.0` 基线快照
> - 远程：无
...
> 本包对这个文件夹的工作结束，只用了一次。`0.1.0` 基线已打在 `governance/baselines/0.1.0/`。
>
> 以后改这个 Skill，对助手说：`按 governance/rules/skill-governance.md 处理，不要直接改。先出 AP。` 不必再调用 skill-devkit。
>
