# 使用示例

这里是完整对话：你怎么开口、助手怎么问、你怎么拍板、文件夹里会出现什么。

人名、Skill 名都是假的，只保留真实用法。贯穿始终的假设定：

```mermaid
flowchart TB
    A["作者林夏"] --> B["空文件夹 meeting-notes/"]
    B --> C["对已安装 skill-devkit 的助手说：初始化 skill"]
    C --> D["得到会议速记开发仓"]
    D --> E["之后读本仓 governance/rules\n不再打开 skill-devkit"]
```

要做的 Skill：英文名 `meeting-notes`，显示名「会议速记」，把会议纪要收成决议和行动项。版权人林夏。

## 建议阅读顺序

```mermaid
flowchart LR
    subgraph init ["用本包"]
        I01["01 空文件夹"]
        I02["02 一句话带齐"]
        I03["03 已经是 Skill"]
        I04["04 非空目录"]
        I05["05 名称不合法"]
        I07["07 英文名占用"]
    end
    subgraph after ["本包退场"]
        I06["06 打包和升版本"]
    end
    I01 --> I02
    I01 --> I03
    I01 --> I04
    I01 --> I05
    I01 --> I07
    I01 --> I06
```

| 文件 | 看什么 | 你开口可以这样说 |
|---|---|---|
| [01-初始化空文件夹.md](01-初始化空文件夹.md) | 主路径：问完再写盘 | 「初始化 skill」 |
| [02-一句话带齐信息.md](02-一句话带齐信息.md) | 说过的字段不再问 | 「开发一个 skill，英文名 meeting-notes，把纪要收成行动项」 |
| [03-已经是Skill仓.md](03-已经是Skill仓.md) | 有 `SKILL.md` 就拒绝再初始化 | 「再初始化一下」 |
| [04-非空目录.md](04-非空目录.md) | 非空直接停止，不写盘 | 「把这个文件夹初始化成 skill」 |
| [05-名称不合法.md](05-名称不合法.md) | 中文名 / 空格过不了 | 先说「会议速记」当英文名 |
| [07-英文名已被占用.md](07-英文名已被占用.md) | 与已装 Skill 撞名先问 | 英文名先说 `pdf` |
| [06-初始化之后打包和升版本.md](06-初始化之后打包和升版本.md) | 本包使命结束，用仓内 `governance/` | 「把版本升到 0.2.0 再打包」 |

助手提问的规矩（所有示例都这样）：

```text
一次只问一项（名字、干什么、显示名、版权、触发说法、远程）。
最后才给默认清单和 description 草稿，你改完再说「按这个写」。

已经说过的字段，助手不会再问。
```
