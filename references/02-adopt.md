...
2. 已有 `governance/rules/skill-governance.md`（已规范）
3. 有 `governance/` 但无上条指纹（来源不明，不覆盖）
4. 工作区是本包（根 `SKILL.md` 的 `name: skill-devkit`，或存在 `assets/seed/skill-governance.md`）
5. 路径位于 `{GROK_HOME 或 ~/.grok}/skills/` 或 `bundled/skills/`
...
| `scripts/`、`assets/`、`tests/` 已有文件 | 不覆盖；缺目录才建；缺 `.gitkeep` / `tests/README.md` 才补 |
...
| `governance/` | **必须不存在**才整棵创建。存在则已在 §2 停止 |
...
整棵 `governance/` 用本版种子（含 §13 `outputs/`、§2.11 B 随 AP 删、§14 双路径口令）。

占位符与初始化相同：`__NAME__` `__DISPLAY_NAME__` `__DESCRIPTION__` `__COPYRIGHT__` `__YEAR__` `__DATE__`。`__VERSION__` 用采用号。

## 6. 写完后顺序

1. `python governance/scripts/snapshot_baseline.py` → `governance/baselines/{VERSION}/`。目录名 = 根 `VERSION`。已有该目录则脚本失败，中止，列出已写文件，不要在半套 `governance/` 上补打。
...
中途失败：列出已写 / 未写。禁止为过检查而改用户技能正文。禁止半套 `governance/` 打补丁。重来：回退该次 commit 或删掉未提交的 `governance/`。
